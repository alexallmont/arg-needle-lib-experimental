...
std::tuple<Eigen::MatrixXd, Eigen::MatrixXd, Eigen::VectorXi> association_mutation_fast(const ARG& arg, const Eigen::MatrixXd& in_mat) {
  ...
  for (int i : arg.leaf_ids) {
    ...
    node_result_map.insert({i, std::make_pair(std::make_tuple(leaf_result, hetero_carrier, 1), arg.end)});
    expiration_pq.push(std::make_pair(arg.end, i));
  }
  ...
  for (const std::unique_ptr<Mutation> &mutation_unique_ptr :
    ...
    while (!postorder.empty()) {
      ...
      for (const ARGEdge *child_edge : child_edges) {
        const ARGNode *child_node = child_edge->child;
        node_result += std::get<0>(node_result_map.at(child_node->ID).first);
        node_het_carrier ^= std::get<1>(node_result_map.at(child_node->ID).first);
        node_desc_count += std::get<2>(node_result_map.at(child_node->ID).first);
      }
      node_result_map.insert(
          {node->ID, std::make_pair(std::make_tuple(node_result, node_het_carrier, node_desc_count), expire_position)});
      expiration_pq.push(std::make_pair(expire_position, node->ID));
    }
